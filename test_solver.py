...
"""
from solver import (PuzzleState, TubeState, ColorSegment, solve, can_pour,
...
def create_simple_puzzle():
    ...
    tubes = [
        TubeState(tube_index=1,
                  segments=[ColorSegment('red', 50),
                            ColorSegment('blue', 50)],
                  capacity_px=100),
        TubeState(tube_index=2,
                  segments=[ColorSegment('blue', 50),
                            ColorSegment('red', 50)],
                  capacity_px=100),
...
def test_can_pour():
    ...
    dest_empty = TubeState(tube_index=2,
                           segments=[ColorSegment('empty', 100)],
                           capacity_px=100)
    assert can_pour(source,
                    dest_empty), "Should be able to pour into empty tube"

    # Valid destination: same color with space
    dest_same_color = TubeState(
        tube_index=3,
        segments=[ColorSegment('empty', 50),
                  ColorSegment('red', 50)],
        capacity_px=100)
    assert can_pour(source,
                    dest_same_color), "Should be able to pour into same color"
    ...
    dest_diff_color = TubeState(
        ...
        segments=[ColorSegment('empty', 50),
                  ColorSegment('blue', 50)],
        capacity_px=100)
    assert not can_pour(source,
                        dest_diff_color), "Should not pour red into blue"

    # Invalid: no space
    dest_no_space = TubeState(tube_index=5,
                              segments=[ColorSegment('red', 100)],
                              capacity_px=100)
    assert not can_pour(source,
                        dest_no_space), "Should not pour into full tube"
...
def test_apply_move():
    ...
    assert new_state.tubes[
        0].top_color.color == 'blue', "Red should be removed from tube 0"
    assert new_state.tubes[
        2].top_color.color == 'red', "Red should be in tube 2"

    # Original state should be unchanged
    assert state.tubes[
        0].top_color.color == 'red', "Original state should not change"
...
def test_apply_move_duplicate_segments():
    ...
    assert len(
        new_state.tubes[0].segments
    ) == 4, f"Should have 4 segments, got {len(new_state.tubes[0].segments)}"
    assert new_state.tubes[
        0].top_color.color == 'blue', "Blue should now be on top"

    # Check that the other red segment is still there
    red_segments = [
        seg for seg in new_state.tubes[0].segments if seg.color == 'red'
    ]
    assert len(
        red_segments
    ) == 1, f"Should still have 1 red segment, got {len(red_segments)}"
    assert red_segments[0].height == 25, "Red segment should be 25px"

    # Verify the poured red is in tube 2
    assert new_state.tubes[
        1].top_color.color == 'red', "Red should be in tube 2"
    assert new_state.tubes[
        1].top_color.height == 25, "Poured red should be 25px"
...
def test_lvl52_solver():
    """Test solver on lvl52 puzzle"""
    print("Testing solver on Level 52 puzzle...")
    ...
    if solution is not None:
        ...
        assert current.is_solved(), "Solution should result in solved state"
        print("✓ Level 52 solver test passed\n")
    else:
        print("  No solution found (may need more depth or better heuristics)")
        print("  This is okay for very complex puzzles\n")
